		...
		<div class="ui_text_indent">
			<div id="box_border">
				<div class="box_bottom">
					...
					<label class="input_label">活动名称：</label>
					<input id="activity" name="contact.activity" value="${contact.activity}" type="text" class="input_text" />
					<label class="input_label">活动时间：</label>
					<input id="activityTime" name="contact.activityTime" value="${contact.activityTime}" 
					onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" type="text" class="input_text" />
					...
					<c:otherwise>
						<c:forEach var="bean" items="${pm.datas}">
							<tr>
								...
								<td>${bean.activityAddress}</td>
								<%-- <td>${bean.contactRemark}</td> --%>
								<td>
								 <a href="javascript:void(0)" onclick="look('${bean.contactID}')">查看</a>&nbsp
								 <c:forEach var="op" items="${blist}">
								        <c:if test="${op.CValue eq 8}">
									       <a href="javascript:void(0)" onclick="toUpdateContact('${bean.contactID}')">编辑</a>&nbsp
									    </c:if> 
