		...
		<div class="ui_text_indent">
			<div id="box_border">
				<div class="box_bottom">
					...
					<select id="fstate" name="fileNamesLog.userId">
						<option value="">--</option>
						<c:forEach items="${userList}" var="user">
							
							<c:if test="${user.userID==fileNamesLog.userId}">
					...
					<c:otherwise>
						<c:forEach var="bean" items="${pm.datas}">
							<tr>
								...
								<c:if test="${bean.fstate==2}">
									<a href="javascript:void(0)"  onclick="rvsign('${bean.fileId}')">标记</a>
								</c:if>
								
								<a href="javascript:void(0)" onclick="lookFilesLog('${bean.id}')">查看
									<c:if test="${bean.remark!=''}">(注)</c:if>
								</a>
