			...
			<div id="box_border">
				<div class="box_bottom">
					...
					<c:forEach items="${blist}" var="blist">
						<input id="c${blist.flag}" value="${blist.flag}" type="hidden"/>
					</c:forEach>
					<input  id="c" value="${fileNames.userId}" type="hidden"/>
					<input name="fileNames.userId" type="checkbox" value="21" id="cks"/>未标记&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="1" id="ck1"/>标记&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="2" id="ck2"/>信息正确&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="3" id="ck3"/>画册退回&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="4" id="ck4"/>待确认&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="5" id="ck5"/>不是负责人&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="6" id="ck6"/>已离职/退休/调离&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="7" id="ck7"/>撤销&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="8" id="ck8"/>跟进信息正确&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="9" id="ck9"/>未寄画册/默认正确&nbsp
					<input name="fileNames.isdelete" type="checkbox" value="10" id="ck10"/>未接电话&nbsp
					</br>
						...
						<c:forEach var="bean" items="${pm.datas}">
							<tr>
								...
								<c:if test="${bean.fstate==2}">
									...
									<a href="javascript:void(0)"  onclick="rvsign('${bean.id}')">标记</a>&nbsp  
									<a href="javascript:void(0)" onclick="toMeet('${bean.id}')">会面</a>   
								</c:if>&nbsp 
								<c:choose>
...
<script>
	window.onload = function(){
	  ...
	  var cks= $('#c').val();
	  if(cks==21){
	  	 var objs = document.getElementById('cks');
		 objs.checked = true;
	  }
	};
