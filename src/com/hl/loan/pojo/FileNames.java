...
public class FileNames implements Serializable {
	...
	@Formula("(select c.down_time from Excel c where c.file_id = id and  c.clss=1 and c.status!=8 and c.status!=7 order by c.id desc limit 0,1)")
	...
	@Formula("(select c.UserName from fileGets c where c.fileId = id )")
