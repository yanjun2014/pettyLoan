    ...
    if (file != null) {
      ...
      if ((file.getUserId() != null) && (!file.getUserId().equals(""))) {
    	  if ((file.getIsdelete() != null) && (!file.getIsdelete().equals("") )) {
    		  exsql=exsql+" and status not in ("+file.getIsdelete()+")";
    	  }else{
    		  exsql=exsql+" and status  in (1,2,3,4,5,6,7,8,9)";
    	  }
     	  flag=1;
      }else{
    	  if ((file.getIsdelete() != null) && (!file.getIsdelete().equals("") )) {
    		  exsql=exsql+" and status in ("+file.getIsdelete()+")";
    		  flag=1;
    	  }
      }
      if(flag==1){
    	  ...
    	  if ((file.getUserId() != null) && (!file.getUserId().equals(""))){
      		  hql.append(" and id not in (:list)");
      		  chql.append(" and id not in (:list)");
      	  }else{
