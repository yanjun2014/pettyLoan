...
public class ExcelOut {
		...
		for (FileNames file : list) {
			...
			for (short k = 0; k < titleLength.intValue(); k = (short) (k + 1)) {
				if (k == 0)
					...
					row.createCell(k).setCellValue(file.getUserName() != null ? String.valueOf(file.getUserName()) : "");
				}
	...
	public static void excel2(List<FileNames> list,
			HttpServletResponse response, String fileName) throws IOException {
		String[] title = { "编号", "邮编", "地址", "单位名称", "部门", "联系人", "性别", "职务","办公电话", "手机", "QQ", "邮箱", "家庭电话", "生日", "备注", "网址" ,"公司类型","是否导入修改","操作人"};
